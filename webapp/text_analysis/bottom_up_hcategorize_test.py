...
# all_text=[]
# for i in range(1,11):
#     all_text.append(open('sow_files_text/'+str(i)+'.txt','r').read())
# tfidftransformer=TfidfVectorizer(ngram_range=(1,1), stop_words='english', tokenizer=clean_ngram)
# X=tfidftransformer.fit_transform(all_text) # using both desc and tile to predict
# Z = [x for _,x in sorted(zip(tfidftransformer.idf_,tfidftransformer.get_feature_names()))]
...
# todo:
# 2. how to find keywords importances
# 3. how good is tf-idf in this use case where there are multiple categories of topics
# 4. to get a good cutoff anomaly detection based on the bell curve
# 5. why does including title creates these extra nodes on the subgroup level!!
# 6  create a small dummy example to understand why the eucledian with l2 norm is not working as cosine!!
# 7. Now there is a word say 'link' in both texts. Now, contextually it can mean different things, So how do we give a score to
#   the similarity between these words based on the context!
# 8. how much the words that are not common affect the scores!


# try out the small paragrams from me and giuseppe
# remove more stop words that are very generic



# some observations:
# 1. when put the whole pdf for an sow, the top words that dominate are words like requirements, baseline, resources etc.
# these extaneaous words that don't really represent any real topical information. They either need to be removed,
# to have their importances lowered somehow:
#   create a chart of the ranking of the most frequent words at a document level
#   (where each page could be a document!! or each paragraph or each document itslef)
# 2. (Done) remove the numbers and other things like pronunds and stuff (do really good cleaning!)
# 3. there should be some word importances based on category for the stahdards that crisply represent that category, some uniqueness.
#   like phi coefficent or something (also maybe based on correlations between the similar standards!!! like if two stadards
#   have similar themes then not reducing them for both as much !! is this implicit in phi cefficient?)
# sows:
# aviation security one has really good results
# software assuarance is good
# try an avaition one from the website
# could not open the sow one (could be interesting terms of electronics)
